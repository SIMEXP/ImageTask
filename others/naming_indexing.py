...
def naming_indexing(fname):
    ...
    '''
    refs = pd.read_csv(os.path.relpath('../sources.csv'))['reference'].tolist()
    urls = pd.read_csv(os.path.relpath('../sources.csv'))['URL'].tolist()
    imageinfos_to_df = []
    ...
    for allpics in os.walk(join(getcwd(), fname)):
        ...
        for picture in tqdm(allpics[2]):
            if os.path.isfile(join(allpics[0], picture)):
                ...
                counter += 1
    imageinfos_to_df = pd.DataFrame(imageinfos_to_df, columns=['name', 'website', 'url',
